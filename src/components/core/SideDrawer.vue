<template>
  <Sidebar variant="sidebar" collapsible="icon" class="border-r border-subtle-100 bg-red-200!">
    <!-- Header with Logo and Collapse Button -->
    <SidebarHeader class="bg-base-secondary-950 border-b border-gray-100">
      <div class="flex items-center justify-between px-3 py-4">
        ...
        <button @click="toggleSidebar"
          class="w-7 h-7 flex items-center justify-center border border-base-secondary-200 rounded-md hover:bg-white/10 transition-colors text-white group-data-[collapsible=icon]:hidden"
          aria-label="Collapse sidebar">
          <Icon name="chevron-left" class="w-4 h-4" />
        </button>
        <!-- Expand button when collapsed -->
        <button v-if="isCollapsed" @click="toggleSidebar"
          class="w-7 h-7 flex items-center justify-center border border-base-secondary-200 rounded-md hover:bg-white/10 transition-colors text-white"
    ...
    <SidebarFooter class="bg-nexum-100">
      <div class="px-4 py-4 text-sm text-subtle-200 text-center group-data-[collapsible=icon]:hidden">
        Mercans © 2025
      </div>
    </SidebarFooter>
  </Sidebar>

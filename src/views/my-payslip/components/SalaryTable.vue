    ...
    <table class="w-full">
      <thead>
        <tr class="border-b border-nexum-50 pb-5!">
          ...
          <th class="px-4 py-4 text-left text-xs font-semibold text-base-secondary-850 min-w-[120px]">
            Period
          </th>
          <th class="px-4 py-4 text-left text-xs font-semibold text-base-secondary-850">
            Payslips
          </th>
          <th class="px-4 py-4 text-left text-xs font-semibold text-base-secondary-850 min-w-[130px]">
            Gross salary
          </th>
          <th class="px-4 py-4 text-left text-xs font-semibold text-base-secondary-850 min-w-[130px]">
            Net pay
      ...
      <tbody>
        <template v-for="(payslip, index) in payslips" :key="payslip.fileAttachment.id">
          <!-- Main Row -->
          <tr class="group hover:bg-nexum-100 transition-colors cursor-pointer"
            ...
            <BaseTooltip content="Expand" side="top">
              <td
                ...
                <button
                  ...
                  <svg class="w-4 h-4 cursor-pointer transform transition-transform duration-200"
                    :class="{ 'rotate-180': expandedRows.has(payslip.fileAttachment.id) }" viewBox="0 0 24 24"
